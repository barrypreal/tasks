...
export function isValid(question: Question, answer: string): boolean {
    if (question.type == "short_answer_question") {
        ...
        const lowerCased = question.options.map((possible: string): string =>
            possible.toLowerCase()
        );
        if (
            lowerCased.findIndex(
                (possible: string): boolean => possible == answer
            ) != -1
        ) {
            return true;
        } else {
...
export function toMarkdown(question: Question): string {
    let questionText: string = "# " + question.name + "\n" + question.body;
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    const newQuestion: Question = { ...oldQuestion };
    newQuestion.id = id;
    newQuestion.published = false;
